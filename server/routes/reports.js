...
const { applyRiskDelta, createAnomalyAlert, scorePatientAssessment } = require('../services/riskEngine');
...
const buildDailyInsights = ({ todayLog, recentLogs }) => {
  ...
  const deltas = compareWindows(recentLogs);
  const formatDelta = (label, delta) => `${label} ${delta > 0 ? 'increased' : 'decreased'} by ${Math.abs(delta).toFixed(0)}% over the last 2 weeks.`;

  if (Math.abs(deltas.sleepPoorDelta) >= 10) {
    insights.push(formatDelta('Poor sleep', deltas.sleepPoorDelta));
    recommendations.push(deltas.sleepPoorDelta > 0
      ? 'Tighten bedtime routine and reduce evening stimulation.'
      : 'Keep the sleep routine stable and consistent.');
  }

  if (Math.abs(deltas.confusionHighDelta) >= 8) {
    insights.push(formatDelta('Confusion episodes', deltas.confusionHighDelta));
    recommendations.push(deltas.confusionHighDelta > 0
      ? 'Use stronger orientation cues and simplify the environment.'
      : 'Maintain current orientation supports.');
  }

  if (Math.abs(deltas.activityLowDelta) >= 8) {
    insights.push(formatDelta('Low activity days', deltas.activityLowDelta));
    recommendations.push(deltas.activityLowDelta > 0
      ? 'Add short guided movement blocks during the day.'
      : 'Keep the current movement routine stable.');
  }

  if (Math.abs(deltas.medicationMissedDelta) >= 8) {
    insights.push(formatDelta('Missed medication days', deltas.medicationMissedDelta));
    recommendations.push(deltas.medicationMissedDelta > 0
      ? 'Strengthen medication reminders and confirmation checks.'
      : 'Keep medication workflow unchanged and monitor.');
  }

  if (Math.abs(deltas.moodAgitatedDelta) >= 8) {
    insights.push(formatDelta('Agitated mood days', deltas.moodAgitatedDelta));
  }

  if (todayLog.gotLost) {
    insights.push('Disorientation / wandering was reported today.');
  ...
  if (todayLog.confusionLevel === 'moderate' || todayLog.confusionLevel === 'severe') {
    insights.push('Today’s confusion level is elevated.');
  }

  if (todayLog.activity === 'low') {
    insights.push('Today’s activity is low relative to the recent pattern.');
  }

  let trend = 'stable';
  const worseningSignals = [deltas.sleepPoorDelta, deltas.confusionHighDelta, deltas.activityLowDelta, deltas.medicationMissedDelta, deltas.moodAgitatedDelta].filter((delta) => delta > 0).length;
  const improvingSignals = [deltas.sleepPoorDelta, deltas.confusionHighDelta, deltas.activityLowDelta, deltas.medicationMissedDelta, deltas.moodAgitatedDelta].filter((delta) => delta < 0).length;

  if (worseningSignals >= 3) trend = 'worsening';
  else if (improvingSignals >= 3) trend = 'improving';

  if (!insights.length) {
    insights.push('No major short-term change detected versus the prior window.');
  }
...
const compareWindows = (logs = []) => {
  const pivot = Math.max(3, Math.floor(logs.length / 2));
  const baseline = logs.slice(0, pivot);
  const recent = logs.slice(-pivot);

  const rate = (rows, predicate) => (rows.filter(predicate).length / Math.max(1, rows.length)) * 100;

  return {
    sleepPoorDelta: rate(recent, (log) => log.sleep === 'poor') - rate(baseline, (log) => log.sleep === 'poor'),
    confusionHighDelta: rate(recent, (log) => log.confusionLevel === 'moderate' || log.confusionLevel === 'severe') - rate(baseline, (log) => log.confusionLevel === 'moderate' || log.confusionLevel === 'severe'),
    activityLowDelta: rate(recent, (log) => log.activity === 'low') - rate(baseline, (log) => log.activity === 'low'),
    medicationMissedDelta: rate(recent, (log) => log.medication === 'missed') - rate(baseline, (log) => log.medication === 'missed'),
    moodAgitatedDelta: rate(recent, (log) => log.mood === 'agitated') - rate(baseline, (log) => log.mood === 'agitated')
  };
};
...
router.post('/daily-log', async (req, res) => {
  try {
    ...
    const recentLogsDescending = await DailyHealthLog.find({ patientId }).sort({ date: -1 }).limit(14).lean();
    const recentLogs = [...recentLogsDescending].reverse();
    const analysis = buildDailyInsights({ todayLog: log, recentLogs });

    const io = req.app.get('io');
    const mlAssessment = await scorePatientAssessment({
      patient,
      log,
      recentLogs
    });

    const delta = Math.round(mlAssessment.riskScore - Number(patient.riskScore || 0));
    await applyRiskDelta({
      ...
      metadata: {
        ...
        food: automaticSignals.food,
        activity: automaticSignals.activity,
        mlRiskScore: mlAssessment.riskScore,
        mlRiskLevel: mlAssessment.riskLevel,
        mlRiskSource: mlAssessment.source
      }
    });

    if (mlAssessment.anomaly?.is_anomaly) {
      await createAnomalyAlert({ io, patient, anomaly: mlAssessment.anomaly });
    }
    ...
    const report = await Report.findOneAndUpdate(
      { patientId, reportType: 'daily', startDate: logDate, endDate: logDate },
      {
        ...
        averageRiskScore: Number(mlAssessment.riskScore || 0),
        riskTrend: analysis.trend === 'worsening' ? 'declining' : analysis.trend,
        totalEvents: 1,
        mlAssessment: {
          riskScore: Number(mlAssessment.riskScore || 0),
          riskLevel: String(mlAssessment.riskLevel || ''),
          source: String(mlAssessment.source || 'heuristic'),
          anomaly: {
            isAnomaly: Boolean(mlAssessment.anomaly?.is_anomaly || false),
            anomalyScore: mlAssessment.anomaly?.anomaly_score == null ? null : Number(mlAssessment.anomaly.anomaly_score)
          }
        },
        createdBy: req.user?.id || null
      },
    ...
    res.json({
      ...
      report: {
        ...
        recommendations: analysis.recommendations,
        note: 'This is a supportive ML-assisted report, not a medical diagnosis.'
      },
      mlAssessment: {
        riskScore: mlAssessment.riskScore,
        riskLevel: mlAssessment.riskLevel,
        source: mlAssessment.source,
        anomaly: mlAssessment.anomaly || null
      }
    });
  } catch (error) {
...
router.get('/patient-patterns/:patientId', async (req, res) => {
  try {
    ...
    const windowSize = Math.max(3, Math.floor(logs.length / 2));
    const baseline = logs.slice(0, windowSize);
    ...
    const trendPatterns = [
      {
        key: 'sleepWorsening',
        count: Math.max(0, Math.round(rate(recent, (log) => log.sleep === 'poor') - rate(baseline, (log) => log.sleep === 'poor'))),
        label: 'Poor sleep worsening'
      },
      {
        key: 'activityDecreasing',
        count: Math.max(0, Math.round(rate(recent, (log) => log.activity === 'low') - rate(baseline, (log) => log.activity === 'low'))),
        label: 'Activity decreasing'
      },
      {
        key: 'confusionIncreasing',
        count: Math.max(0, Math.round(rate(recent, (log) => log.confusionLevel === 'moderate' || log.confusionLevel === 'severe') - rate(baseline, (log) => log.confusionLevel === 'moderate' || log.confusionLevel === 'severe'))),
        label: 'Confusion increasing'
      },
      {
        key: 'medicationMissed',
        count: Math.max(0, Math.round(rate(recent, (log) => log.medication === 'missed') - rate(baseline, (log) => log.medication === 'missed'))),
        label: 'Medication adherence dropping'
      }
    ...
    if (topPattern === 'poorSleepAgitation' || topPattern === 'sleepWorsening') recommendation = 'Prioritize sleep stabilization to reduce agitation episodes.';
    if (topPattern === 'missedMedicationConfusion' || topPattern === 'medicationMissed') recommendation = 'Strengthen medication reminders and adherence checks.';
    if (topPattern === 'lowActivityAgitation' || topPattern === 'activityDecreasing') recommendation = 'Introduce brief structured movement sessions during the day.';
    if (topPattern === 'highConfusionLost' || topPattern === 'confusionIncreasing') recommendation = 'Enable stronger orientation prompts and caregiver escort protocols.';
...
router.get('/history/:patientId', async (req, res) => {
  try {
    ...
    const reports = await Report.find({ patientId })
      .select('reportType startDate endDate generatedAt riskTrend deteriorationRate totalAlerts averageRiskScore mlAssessment')
      .sort({ generatedAt: -1 })
      .limit(20)
      .lean();
