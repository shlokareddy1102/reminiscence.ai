...
const { ensureCaregiverDemoCohort } = require('./demoCohortService');
...
const ensureDemoData = async () => {
  ...
  const caregiverUsers = await User.find({ role: 'caregiver' }).lean();
  for (const caregiver of caregiverUsers) {
    await ensureCaregiverDemoCohort(caregiver);
  }
