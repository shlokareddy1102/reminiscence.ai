...
const seedDemoKnownPeople = (process.env.SEED_DEMO_KNOWN_PEOPLE || 'false').toLowerCase() === 'true';
const allowDestructiveSeed = (process.env.ALLOW_DESTRUCTIVE_SEED || 'false').toLowerCase() === 'true';

const seedDatabase = async () => {
  try {
    if (!allowDestructiveSeed) {
      console.error('Destructive seed blocked. Set ALLOW_DESTRUCTIVE_SEED=true to continue.');
      console.error('This protection prevents accidental deletion of real patient/caregiver data.');
      process.exitCode = 1;
      return;
    }
    ...
    console.log(`Known people seed mode: ${seedDemoKnownPeople ? 'demo placeholders enabled' : 'disabled (upload real faces from caregiver UI)'}.`);
