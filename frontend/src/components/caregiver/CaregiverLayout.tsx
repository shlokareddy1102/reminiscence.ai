...
import { enablePatientPreview } from "@/lib/auth";
...
export default function CaregiverLayout() {
  ...
  const handleOpenPatientPreview = async () => {
    let previewPatientId = selectedPatientId || patients[0]?._id || null;

    if (!previewPatientId) {
      const refreshed = await reloadPatients();
      previewPatientId = Array.isArray(refreshed) ? refreshed[0]?._id || null : null;
    }

    if (!previewPatientId) return;
    setPatient(previewPatientId);
    enablePatientPreview(previewPatientId);
    ...
    <>
      ...
      <div className="p-3 border-t border-sidebar-border space-y-1">
        <button
          ...
          disabled={!selectedPatientId && !patients.length}
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-primary/20 px-3 py-2.5 text-sm font-medium text-primary transition-all duration-200 hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-50"
        >
